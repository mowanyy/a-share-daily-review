...
> 状态：**已实现（v0.37.0，2026-09-09）**——本文档为落地设计与事实记录；实现时一处偏差见「十、实施影响与版本」末尾备注。｜ 编写日期：2026-09-09 ｜ 关联版本：v0.37.0
...
1. **EVAL-005 未全文复用 `is_compliance_risk`**：其实测发现其例外词表（涨停/炸板/跌停…）按**用户提问意图**设计，报告全文扫描时因必然含「涨停」而被豁免、形同虚设；且数据章节的「净买入」含「买入」会误报。实现改为**只扫 LLM 生成章节**（复盘=一总览+七次日预案；plan/open=全文），命中强荐股话术（「建议买入」「荐股」等）→ error，命中交易建议用语（买入/卖出/加仓…）→ warn；数据词（净买入）在数据章节不再误报。
2. **EVAL-110（龙虎榜净买入）未实现**：龙虎榜表为代码拼接章节（A 档），存在性与数值由构造保证，机器回比收益≈0；文档表内标注「可选」故留空。EVAL-111（题材龙头）已实现为 warn 级。
3. **检查级别扩展为四级**：`pass / warn / error / skip`（文档原定 error/warn/skip 三级 + passed 布尔）。CLI 退出码只受 error 级影响（fail_count==0 → 0），warn 不阻断。
4. **入口不自动入库**：`evaluate_report()` 不写库，由 CLI 层调 `AuditDB().log_evaluation()` 入库——测试零污染，接口更干净。
